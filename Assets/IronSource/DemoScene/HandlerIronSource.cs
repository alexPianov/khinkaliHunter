...
public class HandlerIronSource : MonoBehaviour
{
    private ObscuredString _appKey = "18178ed15";

    private AdStatus currentStatus;
    public enum AdStatus
    {
        None, InProcess, Success, Error
    }

    public void Start()
    {
        currentStatus = AdStatus.None;
    ...
    protected async UniTask<AdStatus> ShowInterstitial()
    {
        if (!IronSource.Agent.isInterstitialReady())
        {
            return AdStatus.Error;
        }

        currentStatus = AdStatus.InProcess;
        
        IronSource.Agent.showInterstitial();

        await UniTask.WaitUntil(() => currentStatus == AdStatus.Success || 
                                      currentStatus == AdStatus.Error);

        if (currentStatus == AdStatus.Success)
        {
            IronSource.Agent.loadInterstitial();
        }
    ...
    {
        currentStatus = AdStatus.Success;
